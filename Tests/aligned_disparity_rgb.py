...
with device:
    ...
    while True:
        ...
        if frameRgb is not None and frameDisp is not None:
            ...
            try: 
                if frameDisp.shape[0] == frameRgb.shape[0] and frameDisp.shape[1] == frameRgb.shape[1]:                                
                    blended = cv2.addWeighted(frameRgb, rgbWeight, frameDisp, depthWeight, 0)
                    cv2.imshow(blendedWindowName, blended)
                    frameRgb = None
                    frameDisp = None
                else:
                    raise Exception("Frame size does not match: Disparity: %s x %s - RGB: %s x %s" %
                                    frameDisp.shape[0],frameDisp.shape[1],frameRgb.shape[0] ,frameRgb.shape[1])
            except Exception as e:
                print(e)
                pass
